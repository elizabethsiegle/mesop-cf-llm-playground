...
class State:
  ...
  # Model configs
  selected_model: str = "@hf/thebloke/mistral-7b-instruct-v0.1-awq"
...
def page():
  ...
  with me.box(style=_STYLE_CONTAINER):
    ...
    with me.box(style=_STYLE_CONFIG_COLUMN):
      me.select(
        options=[
          me.SelectOption(label="@cf/meta/llama-3-8b-instruct", value="@cf/meta/llama-3-8b-instruct"),
          me.SelectOption(label="@hf/thebloke/mistral-7b-instruct-v0.1-awq", value="@hf/thebloke/mistral-7b-instruct-v0.1-awq"),
        ],
        label="Model",
        style=_STYLE_INPUT_WIDTH,
...
def on_click_submit(e: me.ClickEvent):
  ...
  state = me.state(State)
  state.response = ""
  for line in generate(state.input, on_model_select):
    state.response += line
    yield
